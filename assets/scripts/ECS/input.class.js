...
export default class Input extends PIXI.utils.EventEmitter {
    ...
    getMousePosition() {
        return {
            x: (this.mousePosition.x / this.canvas.clientWidth) * 2 - 1,
            y: ((this.mousePosition.y / this.canvas.clientHeight) * 2 - 1) * -1
        };
    }

    getMouseDelta() {
        return {
            x: (this.mouseDelta.x / this.canvas.clientWidth) * 2,
            y: (this.mouseDelta.y / this.canvas.clientHeight) * -2
        };
    }
